...
describe("SubscriptionToken", function () {
    ...
    beforeEach(async function () {
        ...
        const SubscriptionToken = await hre.ethers.getContractFactory("SubscriptionToken", owner);
    ...
    it("Should allow users to buy tokens", async function () {
        ...
        // Buy tokens
        await subscriptionToken.connect(user1).buyTokens(tokenAmount, { value: tokenPrice * BigInt(tokenAmount) });
    ...
    it("Should allow users to activate subscriptions", async function () {
        ...
        await subscriptionToken.connect(user1).activateSubscription(user1.address, serviceId1);
        
        // Check if subscription is active
        const isActive = await subscriptionToken.isSubscriptionActive(user1.address, serviceId1);
    ...
    it("Should check and expire subscriptions after the designated time", async function () {
        ...
        console.log("Initial subscription status:", await subscriptionToken.isSubscriptionActive(user1.address, serviceId1));
    ...
    it("Should allow users to request a new platform", async function () {
        ...
        await subscriptionToken.connect(user1).requestNewPlatform(user1.address, newPlatformId);
    ...
    it("Should correctly count the number of subscribers for a platform", async function () {
        ...
        await subscriptionToken.connect(user1).activateSubscription(user1.address, serviceId1);
        await subscriptionToken.connect(user2).activateSubscription(user2.address, serviceId1);
        
        // Check subscriber count
        const subscriberCount = await subscriptionToken.checkSubscribers(serviceId1);
    ...
    it("Should correctly divide costs among subscribers", async function () {
        ...
        await subscriptionToken.connect(user1).activateSubscription(user1.address, serviceId1);
        await subscriptionToken.connect(user2).activateSubscription(user2.address, serviceId1);
        await subscriptionToken.connect(user3).activateSubscription(user3.address, serviceId1);
        
        console.log("Subscribers before division:", await subscriptionToken.checkSubscribers(serviceId1));
    ...
    it("Should allow admin to withdraw funds", async function () {
        ...
        await subscriptionToken.connect(user1).buyTokens(10, { value: tokenPrice * BigInt(10) });
        
        const initialContractBalance = await hre.ethers.provider.getBalance(await subscriptionToken.getAddress());
        ...
        const finalContractBalance = await hre.ethers.provider.getBalance(await subscriptionToken.getAddress());
    ...
    it("Should handle fallback functionality correctly", async function () {
        ...
        await subscriptionToken.connect(user1).fallbackFunction(user1.address, serviceId1);
